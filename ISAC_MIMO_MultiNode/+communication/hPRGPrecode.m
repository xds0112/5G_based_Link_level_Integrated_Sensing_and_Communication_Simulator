...
function [antsym,antind] = hPRGPrecode(siz,nstartgrid,portsym,portind,F)
    ...
    % Calculate 1-based PRG subscripts from RE subscripts, update 'siz' in
    % cases where it is missing a trailing singleton dimension, and create
    % variable 'allplanes' indicating whether or not the RE indices refer
    % to the same locations in all planes
    [prgsubs,~,siz,allplanes] = getPRGSubscripts(siz,nstartgrid,portind,F);

    % Perform precoding to produce antenna symbols and antenna indices
    [antsym,antind] = hPrecode(siz,portsym,portind,F,prgsubs,allplanes);

end

function [prgsubs,crbsubs,siz,allplanes] = getPRGSubscripts(siz,nstartgrid,portind,F)
    ...
    if (size(portind,2)==1 && size(F,1)>1)
        allplanes = false;
    else
        k = subs{1};
        l = subs{2};
...
function [symout,indout] = hPrecode(siz,symin,indin,F,prgsubs,allplanes)
    ...
    if (allplanes)
        K = outdims(1);
        L = outdims(2);
        P = prod(outdims(3:end));
        indout = cast(double(indin(:,1)) + (0:(P-1))*K*L,'like',indin);
        indout = reshape(indout,[size(indin,1) outdims(3:end)]);
        symout = antgrid(indout);
    else
