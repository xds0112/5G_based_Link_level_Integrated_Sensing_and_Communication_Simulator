...
function [TBS,CRC,eqSymbols] = multiDLReceive(carrier,PDSCHs,rx,alg)
    ...
    for ue = 1:numUEs
        ...
        if (alg.PerfectChannelEstimator)
            ...
            H = zeros([K L R P]);
            prgInfo = communication.prgInfo(carrier,pdschExt.PRGBundleSize);
        ...
        if (alg.PerfectChannelEstimator)
            pdschH = communication.prgPrecode(size(H),carrier.NStartGrid,pdschH,pdschHIndices,permute(pdschExt.W,[2 1 3]));
        end
