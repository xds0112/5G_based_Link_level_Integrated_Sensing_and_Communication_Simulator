...
% Ministry of Education, BUPT.
    ...
    NF     = db2pow(bsParams.noiseFigure);
    Teq    = bsParams.antTemperature + 290*(NF-1); % Equivalent noise temperature (in K)
    N0     = fs*kBoltz*Teq;
    Pt     = db2pow(bsParams.txPower-30)*sqrt(waveInfo.Nfft^2/(carrier.NSizeGrid*12*nTxAnts));  % in W
    Ar     = db2pow(bsParams.antGain);
    At     = Ar;
    ...
    ele         = topoParams.elevation;  % Elevation angle of target,  [1 x nTargets]
    azi         = topoParams.azimuth;    % Azimuth angle of target, [1 x nTargets]
