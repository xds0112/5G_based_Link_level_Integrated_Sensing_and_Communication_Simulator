%multiDLReceive Downlink receiver for MU-MIMO
%   [TBS,CRC,EQSYMBOLS] = multiDLReceive(CARRIER,PDSCHs,RX,ALG) performs
...
function [TBS,CRC,eqSymbols] = multiDLReceive(carrier,PDSCHs,rx,alg)
    ...
    for ue = 1:numUEs
        ...
        if (alg.PerfectChannelEstimator)
            ...
            H = zeros([K L R P]);
            prgInfo = communication.dlTransmission.prgInfo(carrier,pdschExt.PRGBundleSize);
        ...
        if (alg.PerfectChannelEstimator)
            pdschH = communication.dlTransmission.prgPrecode(size(H),carrier.NStartGrid,pdschH,pdschHIndices,permute(pdschExt.W,[2 1 3]));
        end
