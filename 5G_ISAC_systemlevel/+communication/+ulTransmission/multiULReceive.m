%multiULReceive Uplink receiver for MU-MIMO
%   [H,NVAR] = multiULReceive(CARRIER,SRSs,RX,ALG) performs uplink
...
function [H,nVar] = multiULReceive(carrier,SRSs,rx,alg)
    ...
    for ue = 1:numUEs
        ...
        if (alg.PerfectChannelEstimator)
            ...
            for p = 1:Nt
                [Hp,nVarRBs(:,p)] = nrChannelEstimate(rxGrid,indices(:,p),symbols(:,p),'CDMLengths',srsCDMLengths(srs));
